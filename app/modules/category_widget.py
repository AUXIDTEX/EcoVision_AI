...
from PyQt6.QtWidgets import QWidget, QVBoxLayout, QLabel, QPushButton, QFrame, QFileDialog, QHBoxLayout
        ...
        if self.file_path:
            ...
            if SelectableImageBox.path[1] == old_path:
                ...
                if pixmap.width() > pixmap.height():
                    ...
                    self.Image_box.image1.setFixedSize(pixmap.size())
                else:
                ...
                if pixmap.width() > pixmap.height():
                    pixmap = pixmap.scaled(w, h, Qt.AspectRatioMode.KeepAspectRatio, Qt.TransformationMode.SmoothTransformation)
                    self.Image_box.image2.setFixedSize(pixmap.size())
                else:
                    pixmap = pixmap.scaled(h, w, Qt.AspectRatioMode.KeepAspectRatio, Qt.TransformationMode.SmoothTransformation)
                    self.Image_box.image2.setFixedSize(pixmap.size())
            ...
            self.Image_box.set_image_path(self.file_path)  # Update the file path for the image box

    

    def delete_category(self):
        if SelectableImageBox.path[1] == self.Image_box.file_path:
            self.Image_box.image1.setPixmap(QPixmap())  # Clear the pixmap
            ...
            self.Image_box.image2.setPixmap(QPixmap())  # Clear the pixmap
        ...
        for image in self.image_array:
            if image["path"] == self.file_path:
                self.image_array.remove(image)  # Remove the image from the array if path matches
                break
